/**
 * 카테고리별 주제 풀 (scripts/lib/topic-pools.mjs → .ts 포팅)
 * Cron에서는 정적 풀만 사용 (동적 AI 생성 제거 → 토큰 절약)
 */
...
export const TOPIC_POOLS: Record<Category, TopicEntry[]> = {
  "꿈해몽": [
...
function getSeasonTag(month: number): string {
 ...
 */
export function pickStaticTopic(
  category: Category,
  usedTitles: string[]
): string | null {
